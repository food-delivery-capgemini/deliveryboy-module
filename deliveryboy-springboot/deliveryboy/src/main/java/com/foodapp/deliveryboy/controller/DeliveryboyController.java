...
public class DeliveryboyController {
	...
	@GetMapping("/deliverboy/bymail/{email}")
	public ResponseEntity<DeliveryboyProfile> getUserDetailsByMail(@PathVariable String email) {
		DeliveryboyProfile profile = service.getUserDetailByMail(email);
		System.out.println(profile);
		if ((profile.getEmail().equals(email))) {
		return new ResponseEntity<DeliveryboyProfile>(profile, HttpStatus.OK);
	}
		return new ResponseEntity<DeliveryboyProfile>(HttpStatus.NOT_FOUND);
